...
def url_gen(urls_df):
    ...
    #this url generator is used to yeild urls to sub_scrapers so they request different urls in parallel
    index = 0

    # while index < urls_df["url"].size():
    #     yield urls_df["url"]
    #     index += 1
...
class MotherScraper:
    ...
    def __init__(self, urls_loc):
        ...
        # intializing sub scrapers

        self.sub_scrapers = [SubScraper(self.url_gen, self.urls_df) for i in range(0, 20)]

    def scrape(self):

        # this method is will execute the scrape methods of all sub_scrapers in parralell
